...
class LDAPOperationTests : XCTestCase {
	...
	func test1_LDAPObjectCreation() throws {
		...
		let op1 = CreateLDAPObjectsOperation(users: [LDAPInetOrgPerson(dn: LDAPDistinguishedName(uid: "test1", baseDN: ldapBaseDNForTests), sn: ["1"], cn: ["test 1"])], connector: connector)
		...
		let op2 = CreateLDAPObjectsOperation(users: [LDAPInetOrgPerson(dn: LDAPDistinguishedName(uid: "test2", baseDN: ldapBaseDNForTests), sn: ["2"], cn: ["test 2"]), LDAPInetOrgPerson(dn: LDAPDistinguishedName(uid: "test1", baseDN: ldapBaseDNForTests), sn: ["1"], cn: ["test 1"])], connector: connector)
	...
	func test2_LDAPObjectRetrieval() throws {
		...
		let op1 = SearchLDAPOperation(ldapConnector: connector, request: LDAPSearchRequest(scope: .children, base: ldapBaseDNForTests, searchQuery: .simple(attribute: LDAPAttributeDescription(string: "uid")!, filtertype: .equal, value: Data("test1".utf8)), attributesToFetch: ["uid", "sn"]))
		try runOperationSync(op1)
		let objects1 = try op1.resultOrThrow().results
		XCTAssertEqual(objects1.count, 1)
		XCTAssertEqual(objects1[0].distinguishedName, LDAPDistinguishedName(uid: "test1", baseDN: ldapBaseDNForTests))
		XCTAssertEqual(objects1[0].attributes["sn"], [Data("1".utf8)])
		XCTAssertNil(objects1[0].attributes["cn"])
		
		let op2 = SearchLDAPOperation(ldapConnector: connector, request: LDAPSearchRequest(scope: .children, base: ldapBaseDNForTests, searchQuery: nil, attributesToFetch: ["uid", "sn"]))
		try runOperationSync(op2)
