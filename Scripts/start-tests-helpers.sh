...
readonly LDAP_INIT_PATH="$(pwd)/TestsData/confs/shared/ldap-docker-init/ldifs"
readonly LDAP_SCHEMAS_PATH="$(pwd)/TestsData/confs/shared/ldap-docker-init/schemas"
...
#docker run --rm -it                              \+
 docker run --detach                              \
	...
	--volume "$LDAP_SCHEMAS_PATH":"/schemas"       \
