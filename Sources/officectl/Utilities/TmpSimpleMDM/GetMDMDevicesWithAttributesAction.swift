...
final class GetMDMDevicesWithAttributesAction : Action<String, Void, [(SimpleMDMDevice, [String: String])]>, SemiSingleton {
	...
	override func unsafeStart(parameters: Void, handler: @escaping (Result<[(SimpleMDMDevice, [String: String])], Error>) -> Void) throws {
		...
		let getAttributesFutures = getDevicesAction.start(parameters: (), weakeningMode: .always(successDelay: 3600, errorDelay: nil), shouldJoinRunningAction: { _ in true }, shouldRetrievePreviousRun: { _, wasSuccessful in wasSuccessful }, eventLoop: eventLoop)
		.flatMap{ devices -> EventLoopFuture<[(SimpleMDMDevice, [String: String])]> in
			let futures: [EventLoopFuture<(SimpleMDMDevice, [String: String])>] = devices.map{ device in
				return self.getDeviceAttributes(token: self.subject, deviceId: device.id, eventLoop: eventLoop).map{ (device, $0) }
			}
			return EventLoopFuture<[(SimpleMDMDevice, [String: String])]>.reduce([], futures, on: eventLoop, +)
		}
		handler(.failure(NotImplementedError()))
	}
