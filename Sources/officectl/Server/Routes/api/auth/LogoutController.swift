...
class LogoutController {
	...
	/** Logging out a given token. Note we don’t do anything here. In theory we
	should save the revoked token and verify token have not been explicitely
	...
	func logout(_ req: Request) throws -> ApiResponse<String> {
		...
		_ = try JWT<ApiAuth.Token>(from: bearer.token, verifiedUsing: .hs256(key: "secret"))
