...
public struct UpdateCAMetricsJob : AsyncScheduledJob {
	
	public let metricName: String
	
	public init(metricName: String = "happn_vault_certs_expiration") {
	...
	public func run(context: Queues.QueueContext) async throws {
		...
		Gauge(label: metricName).destroy()
...
//						certificate.certif.subjectDistinguishedName.flatMap{ ("subject_dn", $0) }
					...
					let gauge = Gauge(label: metricName, dimensions: dimensions)
