...
public extension Authenticator {
	...
	func authenticate(request: RequestType, asyncConfig: AsyncConfig) -> EventLoopFuture<(result: RequestType, userInfo: Any?)> {
		let promise: EventLoopPromise<(result: RequestType, userInfo: Any?)> = asyncConfig.eventLoop.newPromise()
