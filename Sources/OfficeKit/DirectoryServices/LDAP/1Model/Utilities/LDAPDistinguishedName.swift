...
public struct LDAPDistinguishedName {
	...
	public var stringValue: String {
		return values.map{ curValue in
			/* Note: We escape the spaces anywhere in the key and value for
			 *       simplicity, but we only need to escape the spaces if they’re
			 *       the first or last character of the value or key. */
			/* Note: We escape the octothorpe character (“#”) anywhere in the key
			 *       and value for simplicity, but we only need to escape them
			 *       if they’re the first character of the value or key. */
			let replacements = [("\\", "\\\\"), (",", "\\,"), (";", "\\;"), ("<", "\\<"), (">", "\\>"), ("=", "\\="), (" ", "\\ "), ("#", "\\#"), ("\"", "\\\""), ("+", "\\+")]
			return
