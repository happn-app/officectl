				...
				switch self {
					case .waitEndKey:
						switch c {
							...
							case nil:
								throw Err.ldapDNParseFailure(reason:  "Got EOF, expected more key characters.")
						}
					...
					case .waitEndKeyBackslash:
						switch c {
							...
							case nil:
								throw Err.ldapDNParseFailure(reason:  "Got EOF, expected more key characters after a backslash.")
						}
						...
						switch c {
							case .some(let c) where CharacterSet.hexadecimalCharacter.contains(c.unicodeScalars.first!):
								...
								guard let scalar = Unicode.Scalar(intValue) else {
									throw Err.ldapDNParseFailure(reason:  "Cannot convert backslash value \(backslashValue) to unicode scalar")
								}
							...
							default:
								throw Err.ldapDNParseFailure(reason: "Got invalid char or EOF for a numeric LDAP escape.")
						}
					...
					case .waitEndValueBackslash:
						switch c {
							...
							case nil:
								throw Err.ldapDNParseFailure(reason: "Got EOF, expected more value characters after a backslash.")
						}
...
extension LDAPDistinguishedName : Hashable {
	...
	public static func ==(lhs: LDAPDistinguishedName, rhs: LDAPDistinguishedName) -> Bool {
		 ...
		 * This is why we cannot compare the array of values directly :( */
//		return lhs.values == rhs.values
		return (
			lhs.values.count == rhs.values.count &&
			zip(lhs.values, rhs.values).allSatisfy{ $0.0 == $0.1 }
		)
	}
