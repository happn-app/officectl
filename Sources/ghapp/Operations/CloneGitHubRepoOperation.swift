...
class CloneGitHubRepoOperation : RetryingOperation {
	...
	override func startBaseOperation(isRetry: Bool) {
		...
		guard !destinationExists || isDir.boolValue else {
			cloneError = NSError(domain: "com.happn.ghapp", code: 1, userInfo: [NSLocalizedDescriptionKey: "Destination \(destinationURL.path) exists and is a file. Cannot use for cloning repository \(repoName)."])
			baseOperationEnded()
			return
		}
		...
		if !destinationExists {
			...
			guard updateRemoteProcess.terminationStatus == 0 else {
				cloneError = NSError(domain: "com.happn.ghapp", code: 1, userInfo: [NSLocalizedDescriptionKey: "git exited with code \(process.terminationStatus) when updating the remote"])
				return
			}
		...
		if process.terminationStatus != 0 {
			cloneError = NSError(domain: "com.happn.ghapp", code: 1, userInfo: [NSLocalizedDescriptionKey: "git exited with code \(process.terminationStatus)"])
		}
	}
